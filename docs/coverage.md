...
**218 / 234 delivery-ready** (194 full + 24 library wrappers), 16 reduced cores, 0 placeholders.
...
The `mathematical_finance.json` benchmark now has 143 theorems (all `full`). Original 14 + 16 from the first pass + 9 from the second pass + 1 from the third pass + 3 from the fourth pass + 1 from the fifth pass + 6 from the sixth pass + 2 from the seventh pass + 1 from the eighth pass + 12 from the ninth pass + 15 from the tenth pass + 23 from the eleventh pass + 13 from the twelfth pass + 14 from the thirteenth pass + 6 from the fourteenth pass (Phase 10: lognormal second moment / variance + yield curve bootstrap) + 7 from the fifteenth pass (Phase 11: variance swap fair strike + put-K convexity + VaR/CVaR additivity at ρ=1):
...
| `mf-asset-digital-rho` | Asset-or-nothing digital rho | `BlackScholesDigitalGreeks.lean` |
| `mf-cash-digital-rho` | Cash-or-nothing digital rho | `BlackScholesDigitalGreeks.lean` |
| `mf-asset-digital-vega` | Asset-or-nothing digital vega | `BlackScholesDigitalGreeks.lean` |
| `mf-cash-digital-vega` | Cash-or-nothing digital vega | `BlackScholesDigitalGreeks.lean` |
| `mf-asset-digital-theta` | Asset-or-nothing digital theta | `BlackScholesDigitalGreeks.lean` |
...
| `mf-sortino-scale-invariant` | Sortino ratio scale invariance | `PerformanceRatiosExtended.lean` |
| `mf-sortino-translation` | Sortino ratio translation invariance | `PerformanceRatiosExtended.lean` |
| `mf-treynor-scale-invariant` | Treynor ratio scale invariance | `PerformanceRatiosExtended.lean` |
| `mf-information-ratio-scale-invariant` | Information ratio scale invariance | `PerformanceRatiosExtended.lean` |
| `mf-tracking-error-self` | Tracking error vanishes for self-benchmark | `PerformanceRatiosExtended.lean` |
| `mf-tracking-error-cauchy-bound` | Tracking error lower bound from Cauchy-Schwarz | `PerformanceRatiosExtended.lean` |
| `mf-bond-dur-rate-deriv` | Bond duration rate sensitivity ∂_r(D_P·P) = −C_P·P | `BondConvexityImmunization.lean` |
| `mf-bond-2nd-deriv` | Bond second-order rate sensitivity ∂²P/∂r² = C_P·P | `BondConvexityImmunization.lean` |
| `mf-bond-single-conv` | Single-bond portfolio convexity | `BondConvexityImmunization.lean` |
| `mf-bond-2nd-immunization` | Bond portfolio second-order immunization | `BondConvexityImmunization.lean` |
...
| `mf-second-moment-terminal` | Lognormal second moment E[S_T²] | `LognormalSecondMoment.lean` |
| `mf-variance-terminal` | Lognormal variance Var[S_T] | `LognormalSecondMoment.lean` |
| `mf-bootstrap-solve` | Yield curve bootstrap: general solve | `YieldCurveBootstrap.lean` |
| `mf-bootstrap-solve-first` | Yield curve bootstrap: first bond | `YieldCurveBootstrap.lean` |
| `mf-bootstrap-solve-second` | Yield curve bootstrap: second bond | `YieldCurveBootstrap.lean` |
| `mf-bootstrap-consistency` | Yield curve bootstrap: consistency | `YieldCurveBootstrap.lean` |
| `mf-log-forward-bsTerminal` | Log-discount algebraic identity | `VarianceSwap.lean` |
| `mf-integral-log-forward` | Log-moment integral E[log(F/S_T)] = σ²T/2 | `VarianceSwap.lean` |
| `mf-variance-swap-log` | Variance swap fair strike (log-payoff piece) | `VarianceSwap.lean` |
| `mf-gaussianPDF-symmetry` | Standard normal PDF symmetry ϕ(-z) = ϕ(z) | `StrikeConvexityAndRiskAdditivity.lean` |
| `mf-bsP-KK-deriv` | Put price convexity in strike | `StrikeConvexityAndRiskAdditivity.lean` |
| `mf-gaussian-var-additive-rho-one` | VaR additivity at perfect positive correlation | `StrikeConvexityAndRiskAdditivity.lean` |
| `mf-gaussian-cvar-additive-rho-one` | CVaR additivity at perfect positive correlation | `StrikeConvexityAndRiskAdditivity.lean` |

All 143 are `full`, axioms-clean (`#print axioms` = `[propext, Classical.choice, Quot.sound]`).
