...
| `BlackScholes/StrikeConvexity.lean` | `K ↦ max(S − K, 0)` is **antitone** and **convex** in K | `Spreads.lean` (bull-spread = antitone face; butterfly = convex face); `BreedenLitzenberger.lean` (PDF positivity = infinitesimal convex face) |
...
| `Binomial/MertonAmericanCallTree.lean` | Merton 1973 in the binomial tree: one-period continuation of the non-dividend call dominates the forward-discounted intrinsic | Three independent facts assembled: Jensen for `max(·, 0)`, the martingale identity `qu + (1-q)d = e^r`, and the discount-shift identity `e^{−r} · max(S e^r − K, 0) = max(S − K e^{−r}, 0)` |
